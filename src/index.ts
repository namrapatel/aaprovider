...
export class AASigner extends Signer implements TypedDataSigner {
    ...
    async sendUserOperation(transaction: Deferrable<UserOperation>): Promise<string> {
        ...
        const hexUserOp = (<any>this.provider.constructor).hexlifyUserOperation(tx);
...
export class JsonRpcProvider extends BaseProvider {
    ...
    //  - All numeric values zero-striped
    //  - All addresses are lowercased
    static hexlifyUserOperation(transaction: TransactionRequest, allowExtra?: { [key: string]: boolean }): { [key: string]: string | AccessList } {
        // Check only allowed properties are given
        const allowed = shallowCopy(allowedTransactionKeys);
        if (allowExtra) {
            for (const key in allowExtra) {
